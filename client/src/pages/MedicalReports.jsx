...
export default function MedicalReports() {
  ...
  // REDIRECT TO V2 - Auto-forward all users to the new improved version
  useEffect(() => {
    console.log("🔄 [OLD MEDICAL REPORTS] Redirecting to v2...");
    navigate("/medical-reports-v2", { replace: true });
  }, [navigate]);
  ...
  const viewDocument = (document) => {
    ...
    const fileUrl = getUploadUrl(cleanPath);
  ...
  const deleteDocument = async (documentId, fileName) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.MEDICAL_REPORTS}/${documentId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const uploadFiles = async () => {
    ...
    for (const fileObj of files) {
      ...
      try {
        ...
        const response = await fetch(API_ENDPOINTS.MEDICAL_UPLOAD_REPORT, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          },
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() =>
                        window.open(
                          getUploadUrl(
                            (doc.filePath || doc.fileName).replace(
                              /^uploads\//,
                              ""
                            )
                          ),
