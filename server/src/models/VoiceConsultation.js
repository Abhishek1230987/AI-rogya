...
export class VoiceConsultationModel {
  ...
  static async create(consultationData) {
    ...
    const query = `
      INSERT INTO voice_consultations (
        id, user_id, original_message, transcription, detected_language, medical_response, timestamp
  ...
  /**
   * Delete a consultation by id for a specific user
   */
  static async deleteById(consultationId, userId) {
    const query = `
  ...
  /**
   * Delete all consultations for a user
   */
  static async deleteAllByUserId(userId) {
    ...
    try {
      ...
      console.error("Error deleting consultations for user:", error);
      throw error;
    }
  }
  ...
  static async getByUserId(userId, limit = 20, offset = 0) {
    const query = `
      SELECT 
        id, 
        user_id,
        original_message as original_text, 
        transcription as translated_text, 
        detected_language,
        medical_response as ai_response,
